...
from automata.finite_automata import FiniteAutomaton
...
class DFA(FiniteAutomaton):
    ...
    def _validate(self):
        ...
        required_transitions = {(s, a) for s in self.states for a in self.alphabet}
        defined_transitions = {tuple(k.split(',')) if isinstance(k, str) else k for k in self.transitions.keys()}
        missing_transitions = required_transitions - defined_transitions
        if missing_transitions:
            raise ValueError(f"DFA is incomplete. Missing transitions for: {missing_transitions}")
